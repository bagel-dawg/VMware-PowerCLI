...
[CmdletBinding()] 
	...
	Process {
        ...
        if ($FullName) {
            ...
            #Gather scripts using ps1 extension and have a string matching "\VMware\Infrastructure\PowerCLI\Scripts\Initialize-PowerCLIEnvironment.ps1"
	...
	Process {
        ...
        if (!$scripts) {Write-Warning "No PowerShell resources found requiring update within $Path"}
            ...
            foreach ($script in $scripts) {
                ...
                (Get-Content $script.FullPath).Replace("\VMware\Infrastructure\PowerCLI\Scripts\Initialize-PowerCLIEnvironment.ps1","\VMware\Infrastructure\PowerCLI\Scripts\Initialize-PowerCLIEnvironment.ps1") | Set-Content $script.FullPath
